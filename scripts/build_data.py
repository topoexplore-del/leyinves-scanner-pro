...
def detect_patterns(hist, lookback=140):
    """Devuelve hasta 3 patrones detectados: [{name,tipo,icon,conf}].
    Heurísticas honestas: detectan la GEOMETRÍA de la figura; la
    confirmación (ruptura con volumen) sigue siendo trabajo del humano."""
    if len(hist) < 60:
        return []
    ph, pl, nb = find_pivots(hist, k=4, lookback=lookback)
    # Nota: banderas y tazas no requieren pivotes; cada sección valida los suyos
    c = hist["Close"].tail(lookback).reset_index(drop=True)
    close = float(c.iloc[-1])
    found = []
    def add(name, tipo, conf):
        icon = "🟢" if tipo == "bullish" else ("🔴" if tipo == "bearish" else "🟡")
        found.append({"name": name, "tipo": tipo, "icon": icon, "conf": int(conf)})

    # ── Dobles y triples techos/suelos ──
    if len(ph) >= 2:
        (i1, v1), (i2, v2) = ph[-2], ph[-1]
        if i2 - i1 >= 8 and abs(v2 - v1) / max(v1, v2) <= 0.025:
            valley = float(c.iloc[i1:i2 + 1].min())
            if valley <= min(v1, v2) * 0.96 and close < max(v1, v2) * 0.995:
                if len(ph) >= 3 and abs(ph[-3][1] - v1) / v1 <= 0.03:
                    add("Triple techo", "bearish", 75)
                else:
                    add("Doble techo", "bearish", 70)
    if len(pl) >= 2:
        (i1, v1), (i2, v2) = pl[-2], pl[-1]
        if i2 - i1 >= 8 and abs(v2 - v1) / max(v1, v2) <= 0.025:
            peak = float(c.iloc[i1:i2 + 1].max())
            if peak >= max(v1, v2) * 1.04 and close > min(v1, v2) * 1.005:
                if len(pl) >= 3 and abs(pl[-3][1] - v1) / v1 <= 0.03:
                    add("Triple suelo", "bullish", 75)
                else:
                    add("Doble suelo", "bullish", 70)

    # ── Hombro-Cabeza-Hombro (y su inverso) ──
    if len(ph) >= 3:
        (_, s1), (_, hd), (_, s2) = ph[-3], ph[-2], ph[-1]
        if hd > s1 * 1.03 and hd > s2 * 1.03 and abs(s1 - s2) / max(s1, s2) <= 0.04:
            add("Hombro-Cabeza-Hombro", "bearish", 72)
    if len(pl) >= 3:
        (_, s1), (_, hd), (_, s2) = pl[-3], pl[-2], pl[-1]
        if hd < s1 * 0.97 and hd < s2 * 0.97 and abs(s1 - s2) / max(s1, s2) <= 0.04:
            add("HCH invertido", "bullish", 72)

    # ── Líneas de tendencia: triángulos, cuñas, canales, rectángulos ──
    rp = ph[-4:] if len(ph) >= 3 else []
    rl = pl[-4:] if len(pl) >= 3 else []
    sh = _slope_pct(rp, close)
    sl = _slope_pct(rl, close)
    trend60 = (close / float(c.iloc[max(0, len(c) - 61)]) - 1) * 100
    if sh is not None and sl is not None:
        FLAT, RISE = 0.06, 0.10   # % del precio por barra
        h_flat, l_flat = abs(sh) < FLAT, abs(sl) < FLAT
        h_up, l_up = sh > RISE, sl > RISE
        h_dn, l_dn = sh < -RISE, sl < -RISE
        if h_flat and l_up:
            add("Triángulo ascendente", "bullish", 68)
        elif l_flat and h_dn:
            add("Triángulo descendente", "bearish", 68)
        elif h_dn and l_up:
            add("Triángulo simétrico", "neutral", 60)
        elif h_up and l_up:
            if sl > sh * 1.5:
                add("Cuña ascendente", "bearish", 62)
            elif abs(sh - sl) <= max(0.05, 0.35 * abs(sh)):
                add("Canal alcista", "bullish", 65)
        elif h_dn and l_dn:
            if abs(sh) > abs(sl) * 1.5:
                add("Cuña descendente", "bullish", 62)
            elif abs(sh - sl) <= max(0.05, 0.35 * abs(sh)):
                add("Canal bajista", "bearish", 65)
        elif h_flat and l_flat:
            hi = np.mean([p[1] for p in rp]); lo = np.mean([p[1] for p in rl])
            width = (hi - lo) / close * 100
            if 3 <= width <= 15:
                add("Rectángulo " + ("alcista" if trend60 > 0 else "bajista"),
                    "bullish" if trend60 > 0 else "bearish", 58)

    # ── Soportes / resistencias horizontales (≥3 toques) ──
    if len(ph) >= 3:
        last3 = [v for _, v in ph[-3:]]
        if (max(last3) - min(last3)) / np.mean(last3) <= 0.015:
            add("Resistencia horizontal", "neutral", 66)
    if len(pl) >= 3:
        last3 = [v for _, v in pl[-3:]]
        if (max(last3) - min(last3)) / np.mean(last3) <= 0.015:
            add("Soporte horizontal", "neutral", 66)

    # ── Banderas y banderines (impulso + consolidación corta) ──
    for cons in range(6, 16):
        if len(c) < cons + 20:
            break
        seg = c.iloc[-cons:]
        rng = (float(seg.max()) - float(seg.min())) / close * 100
        pole = (float(c.iloc[-cons - 1]) / float(c.iloc[-cons - 13]) - 1) * 100 if len(c) >= cons + 14 else 0
        if rng <= 6:
            half = cons // 2
            ...
            if pole >= 12:
                add(shape + " alcista", "bullish", 64); break
            if pole <= -12:
                add(shape + " bajista", "bearish", 64); break

    # ── Taza con asa (Cup & Handle) ──
    if len(c) >= 70:
        for span in (60, 80, 100):
            ...
            if abs(rim_l - rim_r) / rim_r <= 0.05 and 10 <= depth <= 35 \
               and 0.25 <= bot_pos <= 0.75 and 0 < h_pull <= 8 and close >= rim_r * 0.94:
                add("Taza con asa", "bullish", 66); break

    # deduplicar y quedarnos con lo más confiable
    seen, out = set(), []
...
def fetch_history(ticker, retries=2):
    ...
    for attempt in range(retries + 1):
        try:
            hist = yf.Ticker(ticker.replace(".", "-")).history(period="5y", auto_adjust=True)
...
def get_stock_data(ticker, spy_close=None):
    ...
    try:
        stock = yf.Ticker(ticker.replace(".", "-"))  # Yahoo usa guión: BRK-B, BF-B, HEI-A
        ...
        row = {
            ...
            "signals": scr_signals, "patterns": scr_patterns,
